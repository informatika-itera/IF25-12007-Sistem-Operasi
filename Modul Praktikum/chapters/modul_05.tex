...
\begin{itemize}
    \item Berkas kode sumber C sederhana yang berhasil dikompilasi menjadi \textit{executable}.
...
GCC awalnya merupakan \textit{GNU C Compiler}, namun kini telah berkembang mendukung berbagai bahasa pemrograman seperti C, C++, Objective-C, Fortran, Ada, Go, dan D \cite{gough2004introduction}. GCC adalah standar kompiler yang banyak digunakan dalam sistem operasi mirip UNIX (Unix-like), terutama distribusi Linux. GCC dirancang agar kuat, fleksibel, teroptimasi, dan mendukung banyak arsitektur komponen keras.
...
Kode \textit{assembly} (\texttt{.s}) diproses ke dalam bahasa mesin (\textit{machine level language} berformat biner) yang dapat dimengerti sistem komputer. Perangkat lunak yang merangkap proses ini disebut \textit{Assembler} (\texttt{as}). Output dari tahapan ini adalah berkas objek (\textit{object file}) berekstensi \texttt{.o}. Fail ini masih belum bisa diseksekusi sebelum dihubungkan dengan library atau file objek lain.

\subsubsection{Linking (Penautan)}
Fase terakhir merupakan tahap penggabungan berbagai berkas objek pendukung (\texttt{.o}) dengan fail pustaka bawaan (\textit{library file}). Contoh umum adalah penautan dengan glibc (standar pustaka C). Jika menggunakan pustaka pihak ketiga semisal \textit{math} atau \textit{pthread}, tautannya juga dideklarasikan di sini (\texttt{-lm}, \texttt{-lpthread}). Perangkat lunaknya dinamai \textit{Linker} (\texttt{ld}). Outputnya adalah berkas \textit{executable}.
...
\texttt{-o}     & Spesifikasikan nama berkas output \\ \hline
...
\texttt{-g}     & Sertakan informasi *debugging* (berguna menggunakan GDB) \\ \hline
\end{tabulary}
\end{table}
...
cat salam.s
    ...
    \begin{lstlisting}[language=bash, caption={Assemble menjadi Berkas Objek}]
gcc -c salam.c -o salam.o
    \end{lstlisting}
    `Object file` (\texttt{.o}) ini bukan lagi sebuah teks ASCII yang bisa dibaca manusia, ini adalah sebuah format instruksi mesin dalam bentuk biner. Anda tidak perlu membacanya dengan `cat`.
...
gcc salam.o -o program_salam
    ...
    \item Sekarang Anda bisa langsung mencoba menjalankan (run) berkas executablenya:
